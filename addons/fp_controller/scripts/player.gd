...
@export var STRIDE_LEFT: String = "stride_left"
@export var STRIDE_RIGHT: String = "stride_right"
...
func check_controls() -> void:
	...
	if !InputMap.has_action(STRIDE_LEFT):
		push_error("No control mapped for 'stride_left', using default...")
		_add_input_map_event(MOUSE_BUTTON_LEFT, STRIDE_LEFT)
	if !InputMap.has_action(STRIDE_RIGHT):
		push_error("No control mapped for 'stride_right', using default...")
		_add_input_map_event(MOUSE_BUTTON_RIGHT, STRIDE_RIGHT)
...
func _on_state_machine_transitioned(state: PlayerState) -> void:
	print(state.name)
	is_moving = state is Walk || state is Sprint || state is Stride
