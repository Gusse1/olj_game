...
func handle_input(event: InputEvent) -> void:
	...
	if (Input.is_action_just_pressed("stride_left") || Input.is_action_just_pressed("stride_right")) && player.is_on_floor():
		state_machine.transition_to(state_machine.movement_state[state_machine.STRIDE])
